...
def run_backtest():
    ...
    for i in range(60, len(test_range)):
        ...
        if position == 0:
            if should_enter:
                ...
                log(f"COMPRA (IA: {prob*100:.1f}% | {current_regime}) em {entry_price:.2f} ({timestamp})")
            ...
            # TODO: Make these configurable or match Strategy
            ...
            if reason:
                ...
                log(f"VENDA ({reason}) em {current_price:.2f} | PnL: {pnl_pct*100:.4f}% (Lev: {realized_pnl*100:.2f}%) | Saldo: {balance:.2f}")
    ...
    if len(probs) > 0:
        import numpy as np
        log(f"Estatísticas de Confiança (IA) -> Média: {np.mean(probs):.4f} | Máx: {np.max(probs):.4f} | Mín: {np.min(probs):.4f}")

    log("\n--- Resultados do Backtest (Últimos 500m) ---")
    log(f"Total de Operações: {total_trades}")
    log(f"Vitórias (Wins): {wins}")
    log(f"Derrotas (Losses): {losses}")
    log(f"Taxa de Acerto (Winrate): {winrate:.2f}%")
    log(f"Saldo Final: ${balance:.2f} (Início: $1000.00)")
    log(f"Retorno: {((balance-1000)/1000)*100:.2f}%")
