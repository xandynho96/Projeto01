...
class DeepSeekStrategist:
    ...
    def generate_strategies(self, indicators: List[str], count: int = 3, market_regime: str = "SIDEWAYS", description: str = "") -> List[Dict[str, Any]]:
        """
        Generates trading strategies using DeepSeek LLM, tailored to the specific Market Regime.
        """
        if not self.api_key:
            return []
...
Your goal is to generate high-alpha 'scalping' strategies for Bitcoin (1m timeframe) that work specifically in the requested Market Environment.
...
Rules:
- **PRIORITY**: Always consider Candlestick Patterns (Price Action) as confirmation.
...
Generate {count} distinct, professional scalping strategies specifically optimized for {market_regime}.
...
Think like a institutional algorithm.
"""
