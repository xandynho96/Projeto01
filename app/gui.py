...
from app.core import ai_brain # Force PyInstaller to bundle explicitly (Import BEFORE trader)
import pandas as pd # Force PyInstaller to bundle
from app.trader import BitcoinTrader # Force PyInstaller to bundle
from app.utils import config
from app.utils import script_utils # Make sure this file exists or is handled
from app.core import evolution # Import the evolution module
...
class BitcoinAIApp:
    ...
    def create_widgets(self):
        ...
        self.log_area.tag_config("deepseek_refuse", foreground="#ff8800", font=("Consolas", 10, "bold")) # Orange
        self.log_area.tag_config("highlight_val", foreground="#00e6e6", font=("Consolas", 10, "bold")) # Cyan/Teal for values
    ...
    def load_settings(self):
        ...
        if os.path.exists(os.path.join("config", "user_config_roe.json")):
            try:
                with open(os.path.join("config", "user_config_roe.json"), "r") as f:
                    data = json.load(f)
    ...
    def save_settings(self, api_key, secret, settings):
        ...
        try:
            ...
            with open(os.path.join("config", "user_config_roe.json"), "w") as f:
            ...
            while True:
                ...
                if is_evo:
                    ...
                    if "decisão: buy" in lower_msg or "executando entrada" in lower_msg or "entrada executada" in lower_msg:
                        ...
                        tag = "sell"
                    elif "deepseek rejeitou" in lower_msg or "rejeitou a entrada" in lower_msg:
                        tag = "deepseek_refuse"
                    elif any(x in lower_msg for x in ["error", "crash", "falha", "recusada", "cancelando", "saldo insuficiente"]):
                        tag = "error"
                    elif "holding" in lower_msg or "aguardando" in lower_msg or "neutro" in lower_msg:
                         pass # Normal info
                        
                    self.log_area.insert(tk.END, msg + '\n', tag)
                    
                    # Highlight SL/TP values if present
                    if "sl:" in lower_msg or "tp:" in lower_msg or "alvo dinâmico" in lower_msg:
                         # Simple highlight of the whole line for now, or use regex for values if needed
                         # Let's apply highlight_val tag to the whole line which is clearer for user
                         self.log_area.tag_add("highlight_val", f"{float(self.log_area.index('end-1c'))-1:.1f}0", "end-1c")
    ...
    def load_strategies(self):
        ...
        try:
             ...
             # Filter for ACTIVE (Promoted) strategies only and Sort by Winrate DESC
             strats = dm.get_active_strategies()
             # Sort by Winrate Descending
             strats.sort(key=lambda x: x['winrate'], reverse=True)
    ...
    def toggle_trader(self):
        if not self.running_trader:
            ...
            # Auto-Start Evolution (Background)
            self.log("🤖 Iniciando Evolução Genética Automática (Background)...")
            self.run_evolution_handler()
        else:
            # Stop (Soft)
    ...
    def update_pnl_display(self):
        ...
        try:
            if hasattr(self, 'trader_instance') and self.trader_instance:
                ...
                pnl = self.trader_instance.dm.get_api_pnl_24h()
